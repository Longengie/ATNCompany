...
def login():
    ...
    if  logresults.count() == 1:
        ...
        staffresults = logcollection.find("StaffID": session['ID'])
...
def product():
    ...
    if session['role'] == "Shop":
        ...
        return render_template("product.html", productList = lpro)
    else:
        return redirect(url_for('home'))
    ...
    if session['role'] == "Shop":
        if ("productID" in request.args and "productName" in request.args and "productPrice" in request.args):
            ...
            if  PIresults.count() < 1 or PNresults.count() < 1:
                ### Adding data
                newProduct = {"productID" : pID, "productName" : pName, "productPrice" : pPrice, "Date": today}
...
def order():
    ...
    if session['role'] == "Shop":
        ...
        return render_template("order.html", orderList = lord)
    else:
        return redirect(url_for('home'))
    ...
    if session['role'] == "Shop":
        if ("productID" in request.args and "orderID" in request.args and "orderAmount" in request.args):
            ...
            if  PIresults.count() >= 1 and ORresults.count() < 1:
                ...
                oSum = oNum * pPrice
                newOrder = {"orderID" : oID, "productID" : pID, "orderAmount" : oNum, "orderSum" : oSum, "Department" : pDept, "Date": today}
...
def updateorder():
    ...
    if session['role'] == "Shop":
        if ("productID" in request.args and "orderID" in request.args and "orderAmount" in request.args):
            ...
            PIresults = procollection.find({"productID":pID})
            ORresults = ordcollection.find({"productID":pID, "Department" : pDept})
            if  PIresults.count() >= 1 and ORresults.count() >= 1:
                pPrice = PIresults[0]["productPrice"]
                oSum = oNum * pPrice
...
def deleteorder():
    ...
    if session['role'] == "Shop":
        if ("productID" in request.args):
            ...
            PIresults = procollection.find({"orderID":oID, "Department" : pDep})
            if  PIresults.count() >= 1:
                ### Delete data
...
@app.route('/todayproduct', methods=['GET', 'POST'])
def todayproduct():
    ###Ch-eck role of session, if not the go to home
    if session['role'] == "Director":
        today = date.today()
        procollection = db["products"]
        ###Show product database for today
        lpro = procollection.find({"Date": today})
        return render_template("sumlist.html", List = lpro)
    else:
        return redirect(url_for('home'))
